...
def get_model_params(ml_model_name : str, ml_model) -> NDArrays:
    ...
    if ml_model_name == 'SVM' :
        ...
        params = [
            ml_model.coef_,
            np.asarray([ml_model.intercept_]), # Make intercept a 1D array. Flower expects NDArrays. For LASSO intercept is a float in case of a simple regression (i.e. y is a scalar)
        ]
    else:
        raise ValueError(f"ML algorithm {ml_model_name} not implemented")
