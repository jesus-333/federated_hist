...
I also test this example with the TLS connection. The certificate folder was generated by the script `generate_auth_keys.sh` and `generate_cert.sh` inside the `3_flwr_auth_example/` folder.
Remember also to modify the `pyproject.toml` accordingly.
More info on how to run with TLC connection [here](https://flower.ai/docs/framework/how-to-enable-tls-connections.html).
